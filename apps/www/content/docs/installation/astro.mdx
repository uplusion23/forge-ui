...
When running `bunx --bun forge-ui@latest init`, your tailwind config for content will be overwritten. To fix this, change the `module.exports` to `export default` and the `content` section with the code below to your `tailwind.config.mjs` file:
...
```bash
bunx --bun forge-ui@latest add button
```
